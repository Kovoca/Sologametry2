...
use scale_sim::wip::{
    ...
    reserve_all, rework, start, still_good, what_becomes_of_it, CannotReserve, Disposition,
    Feature, Progress,
};
...
fn a_reserved_sheet_is_still_on_its_rack() {
    ...
    assert!(
        !store.available(sheet),
        "a reserved sheet was free for the taking"
    );
...
fn starting_the_job_puts_it_in_the_press() {
    ...
    assert!(
        !store.available(blank),
        "somebody walked off with a clamped panel"
    );
    ...
    assert!(
        store.available(blank),
        "a cool panel in a tray could not be lifted out"
    );
}
...
fn cutting_a_sheet_ends_it_and_starts_two() {
    ...
    // from.
    assert!(
        store.get(sheet).is_none(),
        "the sheet survived being cut up"
    );
    assert_eq!(store.graves().len(), 1);
    for piece in [blank, offcut] {
        let s = store.get(piece).unwrap().shape.as_ref().unwrap();
        assert!(
            s.lineage.contains(&sheet),
            "a piece does not know what it came off"
        );
    }
    ...
    assert!(t
        .scrap
        .iter()
        .any(|s| s.0 == Material::MildSteel && s.1 > 0.0));
}
...
fn pressing_a_blank_leaves_the_same_object() {
    ...
    assert!(
        (mass(&store, blank) - before).abs() < 1e-9,
        "pressing changed its weight"
    );
    ...
    assert_eq!(
        s.lineage, came_from,
        "the press forgot where the steel came from"
    );
...
fn painting_a_panel_puts_some_of_the_tin_in_the_air() {
    ...
    assert_eq!(
        store.get(panel).unwrap().shape.as_ref().unwrap().surface,
        Surface::Painted
    );
    ...
    assert!(
        (gone - 0.275).abs() < 1e-9,
        "{gone:.3} kg of solvent went missing"
    );
...
fn the_latch_inside_the_door_is_still_that_latch() {
    ...
    assert_eq!(
        rec.as_built,
        vec![latch, hinge, glass],
        "the door does not say what is in it"
    );
    assert_eq!(
        store.placement(latch),
        Some(Placement::Installed {
            host: scale_sim::item::Host::Item(door),
            mount: 0
        })
    );
    assert!(
        !store.available(latch),
        "the latch out of a finished door was on offer as stock"
    );
...
fn melting_ends_the_lineage() {
    ...
    assert!(
        store.get(billet).unwrap().shape.is_none(),
        "a billet remembered being a sheet"
    );
    assert!((mass(&store, billet) - before * 0.97).abs() < 1e-6);
    assert!(
        t.emissions.iter().map(|e| e.1).sum::<f64>() > 0.0,
        "melting lost nothing to the flue"
    );
...
fn what_survives_a_stoppage_depends_on_what_was_happening() {
    ...
    assert!(
        (cutting.fraction() - 0.5).abs() < 1e-9,
        "a cut path healed itself overnight"
    );

    let mut kiln = Progress::Heat {
        celsius: 20.0,
        target_c: 850.0,
        ambient_c: 20.0,
    };
    ...
    assert!(
        kiln.fraction() < 0.7,
        "a kiln held its heat through a two-hour outage"
    );
    ...
    assert!(
        warm.fraction() > 0.2,
        "the glue stopped setting because the lights went out"
    );

    let mut chilled = Progress::Cure {
        reacted: 0.2,
        at_c: 8.0,
        wants_c: 20.0,
    };
    ...
    assert!(
        chilled.fraction() > 0.2,
        "a cold cure stopped dead at eight degrees"
    );
    assert!(
    ...
    assert!(
        (frozen.fraction() - 0.2).abs() < 1e-9,
        "glue went off in a frozen yard"
    );

    // And there is no universal figure: two operations at the same
    // fraction are in completely different states.
    let welding = Progress::Weld {
        done: 3,
        segments: 6,
    };
    let coating = Progress::Coat {
        microns: 20.0,
        target_microns: 40.0,
        layers: 1,
    };
...
fn a_botched_pressing_does_not_give_the_sheet_back() {
    ...
    assert!(
        store.get(sheet).is_none(),
        "a failed pressing put the sheet back on the rack"
    );
    assert!(store.get(panel).is_some(), "the malformed panel vanished");
    assert_eq!(
        store.live(),
        live_before,
        "the failure created or destroyed an object"
    );
    assert!(offcut.is_some(), "the offcut was un-cut");

    let p = store.get(panel).unwrap();
    assert!(
        p.condition.damage > 0.4,
        "a botched panel came out undamaged"
    );
    ...
    assert!(
        t.scrap.iter().any(|s| s.1 > 0.0),
        "the spoiled metal went nowhere"
    );
}
...
fn putting_it_right_does_not_start_again() {
    ...
    assert!(
        after.quality.dimensional_accuracy > bad,
        "the rework improved nothing"
    );
    assert!(
        after.condition.damage < 0.45,
        "it is still as bent as it was"
    );
    ...
    assert!(
        (after.mass_kg - mass_before).abs() < 1e-9,
        "rework consumed the workpiece"
    );
...
fn a_board_carried_off_in_the_night_is_not_booked_any_more() {
    ...
    assert!(
        still_good(&store, 11, &[a, b]).is_empty(),
        "a good booking read as broken"
    );
    ...
    assert_eq!(
        broken,
        vec![a],
        "the booking survived the sheet being taken"
    );
...
fn a_panel_can_be_pressed_or_beaten_and_it_is_still_the_panel() {
    ...
    assert_eq!(
        by_press.0.shape.as_ref().unwrap().becoming,
        by_hand.0.shape.as_ref().unwrap().becoming
    );
...
fn the_owner_and_the_employee_press_the_same_panel() {
    ...
    assert_eq!(
        make(),
        make(),
        "the same work came out differently for a different person"
    );
}
...
fn taking_the_door_apart_gives_back_the_second_hand_latch() {
    ...
    assert!(
        released.contains(&latch),
        "the latch was not among what came off"
    );
    let back = store.get(latch).unwrap();
    assert_eq!(
        back.given_name.as_deref(),
        Some("off the scrapper"),
        "a different latch came out"
    );
    assert!(
        back.condition.wear >= 0.44,
        "the latch was rejuvenated by being unbolted"
    );
    assert!(matches!(
        store.placement(latch),
    ...
    assert!(
        store.get(door).is_none(),
        "the door survived being dismantled"
    );
...
fn a_car_door_from_the_rack_to_the_scrap_heap() {
    ...
    assert_eq!(
        store.get(skin).unwrap().shape.as_ref().unwrap().surface,
        Surface::Painted
    );
    ...
    assert!(
        !store.available(door),
        "an uncollected door was treated as stock"
    );
    ...
    assert_eq!(
        store.get(door).unwrap().quality.overall(),
        quality_then,
    ...
    let (mut van, _) =
        FittedVehicle::adapt(9, Vehicle::van(), &serviceable_parts(), &cat, &mut store);
    van.mounts.push(Mount::new(
        "door aperture",
        (2, 0),
        Fitting::Opening,
        JointMethod::Bolted,
    ...
    let door_mass = mass(&store, door);
    let (recovered, released) =
        dismantle(&mut store, &cat, door, Teardown::Deconstruct, 0.85, 5, 210);
    ...
    assert!(
        store.get(offcut).is_some(),
        "the offcut evaporated somewhere along the way"
    );
...
fn a_door_nobody_collects_holds_the_bay_it_was_made_in() {
    ...
    assert!(
        !store.slot_free(press, 0),
        "the press read as clear with a door standing on it"
    );
    assert!(store.occupants_of(press).contains(&door));

    // **2. The finished object already exists.** Not a promise of one.
    assert!(
        store.get(door).is_some(),
        "the door that could not be moved does not exist"
    );
    assert!(
        mass(&store, door) > 4.0,
        "the door that exists weighs nothing"
    );
    assert_eq!(
        store.status(door),
        WorkStatus::AwaitingUnload { order: 500 }
    );
    assert!(
        !store.available(door),
        "an uncollected door was on offer as stock"
    );
    ...
    assert!(
        store.get(sheet).is_none(),
        "the sheet came back off the rack"
    );
    assert_eq!(
    ...
    assert!(store
        ...
        .unwrap_or(false));
    assert!(
        !store.available(skin),
        "a panel inside a finished door was on offer"
    );
    ...
    assert!(
        store.slot_free(press, 0),
        "the press is still blocked after the door was collected"
    );
    let next = a_sheet(&cat, &mut store);
    let started = start(&mut store, &cat, next, design, 501, press).expect("the press is free now");
    assert_eq!(
        store.placement(started).and_then(|p| p.occupying()),
        Some(press)
    );
    assert!(
        !store.slot_free(press, 0),
        "two jobs are on one press at once"
    );
}
...
fn every_kind_of_progress_survives_being_written_down() {
    ...
    for pl in [
        ...
        Placement::Carried {
            person: 4_000_000_009,
        },
        Placement::Contained {
            container: scale_sim::id::Id::from_bits(0x0000_0003_0000_0002),
        },
        Placement::Installed {
...
fn a_part_finished_panel_reloads_exactly_as_it_was() {
    ...
    assert_eq!(
        back_shape.lineage,
        vec![sheet],
        "it forgot which sheet it came off"
    );
    ...
    assert_eq!(
        again.bytes, bytes,
        "writing it twice gave two different files"
    );
}
...
fn every_stage_a_workpiece_reaches_is_in_the_roster() {
    ...
    for name in [
        ...
        assert!(
            ...
            "the library writes {name} and cannot read it back"
        );
...
fn a_billet_remembers_what_was_charged_into_it() {
    ...
    assert!(
        heat.recycled_fraction > 0.9,
        "a heat of pure scrap read as primary metal"
    );
    assert!(
        !heat.contamination.is_empty(),
        "a mixed charge came out chemically pure"
    );
    ...
    assert!(
        heat2.ancestry() > 3,
        "the second melt lost the first one's charge"
    );
    assert!(
        heat2.hazardous,
        "remelting laundered the hazard out of the record"
    );
}
...
fn the_design_expects_a_regulator_and_this_door_has_that_one() {
    ...
    for want in ["window regulator", "door latch", "door glass"] {
        assert!(
            expects.contains(&want),
            "the design does not expect a {want}: {expects:?}"
        );
    }
    assert!(
        design.bill.declared_mass() > 25.0,
        "the design bill weighs nothing"
    );
    ...
    assert!(
        (mass(&store, door) - parts_mass - 0.04).abs() < 1e-9,
        "the mass was counted twice"
    );
...
fn an_offcut_is_at_the_machine_until_somebody_carries_it_back() {
    ...
    let bench = Placement::Fixtured {
        resource: 6,
        slot: 0,
        clamped: false,
    };
    store.place(sheet, bench);
    let (blank, offcut, _) = cut(
        &mut store,
        &cat,
        sheet,
        4.5,
    ...
    assert_eq!(
        store.placement(offcut),
        Some(bench),
        "the offcut appeared back on the rack"
    );
    ...
    assert!(
        minutes > 1.0,
        "a 27 kg part-sheet went back on the rack instantly"
    );
    assert!(
        minutes < 15.0,
        "putting a sheet away took a quarter of an hour"
    );
    ...
    assert!(
        quick < minutes,
        "a screw took as long to put away as a steel sheet"
    );
    ...
    assert!(
        alone > together,
        "a two-man lift was no easier with two men"
    );
}
...
fn a_thing_nobody_wants_is_not_automatically_buried() {
    ...
    assert_eq!(
        what_becomes_of_it(0.9, true, true, true, false),
