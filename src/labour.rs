...
/// **The posts a town has, by trade**, as its works, shops, services, state
/// and carriers are staffed to run.
///
/// `Workforce::posts` is one figure for the works and shops, which is the
/// question the labour statistics ask. A person deciding what work to go
/// after asks a different one — *is there a place for a cook here* — and
/// until this nothing could answer it. Rated rather than today's, because
/// an opening is a post that exists; whether it is busy this week is what
/// the people working it say about it.
///
/// Every source already counts its own people, so this only sorts them:
/// a works' floor by `SiteKind::worked_by` and its charge hands as
/// supervisors, exactly as `update` counts them; a shop's floor and its
/// managers off its fixtures; the private services by sector; the state's
/// posts as public service; and a carrier's drivers where its depot is.
pub fn posts_by_trade(econ: &Economy) -> Vec<[f64; 13]> {
    use crate::person::Trade;
    let n = econ.markets.len();
    let mut out = vec![[0.0f64; 13]; n];
    let grid_capacity = econ.grid.capacity();
    for site in econ.ledger.sites.iter() {
        if site.market >= n {
            continue;
        }
        let here = &mut out[site.market];
        if let Some(b) = &site.fitted {
            here[Trade::Supervisor.index()] += b.supervisors() + b.managers();
            here[Trade::Shopworker.index()] += b.staff() - b.supervisors() - b.managers();
        }
        let Some(r) = site.recipe else { continue };
        let rated = match site.kind {
            SiteKind::PowerPlant => grid_capacity,
            _ => site.throughput,
        };
        let floor = hands_for(rated, RECIPES[r].labour);
        here[site.kind.worked_by().index()] += floor;
        if floor >= 6.0 {
            let sup = (floor / SPAN_OF_CONTROL).ceil();
            here[Trade::Supervisor.index()] += sup + (sup / SPAN_OF_CONTROL).ceil().max(1.0);
        }
    }
    if let Some(svc) = econ.services.as_ref() {
        for m in 0..n {
            for sector in crate::services::Sector::ALL {
                for &(t, share) in sector.trades() {
                    out[m][t.index()] += svc.posts_in(m, sector) * share;
                }
            }
        }
    }
    for m in 0..n {
        if let Some(gov) = econ.government(m) {
            for service in crate::state::Service::ALL {
                let posts = gov.posts_for(econ, m, service);
                for &(t, share) in service.trades() {
                    out[m][t.index()] += posts * share;
                }
            }
        }
    }
    if let Some(freight) = econ.logistics.as_ref() {
        for c in freight.carriers.iter() {
            if c.home < n {
                out[c.home][Trade::Haulier.index()] += c.drivers();
            }
        }
    }
    out
